 ...
 * 服务器密钥交换消息
 */
final class ServerKeyExchangeMessage extends AbstractHandshakeMessage
{
    /**
     * 消息类型
     */
     ...
     */
    public function setKeyExchangeParams(string $keyExchangeParams): void
    {
    ...
    public static function decode(string $data): static
    {
        ...
        if (strlen($body) - $bodyOffset >= 4) { // 至少需要2+2字节用于签名算法和签名长度
            // 假设剩余部分的开始是签名算法
            $unpacked1 = unpack('n', substr($body, strlen($body) - 4, 2));
            $unpacked2 = unpack('n', substr($body, strlen($body) - 2, 2));
            if (false === $unpacked1 || false === $unpacked2) {
                throw new InvalidMessageException('Failed to unpack signature algorithm or signature length');
            }
            $possibleAlgorithm = $unpacked1[1];
            $possibleSignatureLength = $unpacked2[1];
