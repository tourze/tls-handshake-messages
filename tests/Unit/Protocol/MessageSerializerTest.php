...
namespace Tourze\TLSHandshakeMessages\Tests\Unit\Protocol;
...
class MessageSerializerTest extends TestCase
{
    ...
    {
        $message = $this->createMock(HandshakeMessageInterface::class);
        $message->method('encode')->willReturn($encodeReturn);
        $message->method('getType')->willReturn($type ?? HandshakeMessageType::CERTIFICATE);
        $message->method('isValid')->willReturn(true);
